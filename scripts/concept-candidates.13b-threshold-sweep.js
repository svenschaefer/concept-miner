...
const { buildConceptCandidatesFromStep12 } = require("../src/core/step13");

const STEP13_DIR = __dirname;
const DEFAULT_ARTIFACTS_ROOT = path.resolve(STEP13_DIR, "..", "test", "artifacts");
const DEFAULT_BENCHMARK = path.resolve(STEP13_DIR, "..", "test", "benchmark", "independent.expected-concept-candidates.yaml");
...
function loadStep12ForSeed(artifactsRoot, seedId) {
  const candidates = [
    ...
    path.join(artifactsRoot, seedId, "seed", "seed.elementary-assertions.yaml"),
  ];
  const p = candidates.find((v) => fs.existsSync(v));
  if (!p) {
    throw new Error(`Missing Step12 artifact for ${seedId} under ${artifactsRoot}`);
  }
